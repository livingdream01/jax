...
export async function getStock(symbol: string): Promise<TickerInfo | null> {
  try {
    ...
    const data = await fetchJSON(`https://query1.finance.yahoo.com/v8/finance/chart/${safe}?interval=1d&range=1d`);
    ...
    return { symbol: safe, name: meta.shortName || safe, price, change, changePercent: changePct, high: meta.regularMarketDayHigh, low: meta.regularMarketDayLow, volume: meta.regularMarketVolume };
  } catch { return null; }
}

export async function getCrypto(coinId: string): Promise<TickerInfo | null> {
  try {
    const data = await fetchJSON(`https://api.coingecko.com/api/v3/coins/${coinId.toLowerCase().trim()}?localization=false&tickers=false&community_data=false&developer_data=false`);
    const m = data?.market_data;
    if (!m) return null;
    return { symbol: (data.symbol || coinId).toUpperCase(), name: data.name || coinId, price: m.current_price?.usd, change: +(m.price_change_24h_in_currency?.usd || 0).toFixed(2), changePercent: +(m.price_change_percentage_24h_in_currency?.usd || 0).toFixed(2), high: m.high_24h?.usd, low: m.low_24h?.usd, volume: m.total_volume?.usd };
  } catch { return null; }
}
